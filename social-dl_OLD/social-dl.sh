...
msg() {
    ...
    case "$key" in
        ...
        "error_dangerous_chars")
            ...
            ;;
        "error_not_url")
            [ "$SCRIPT_LANG" = "de" ] && echo "sieht nicht nach URL aus!" || echo "doesn't look like a URL!"
            ;;
        "error_unsupported")
...
show_help() {
    cat << EOF
Video/Audio Downloader for Instagram, Twitter/X, YouTube, Reddit and TikTok

$(msg "help_usage")
    $0 [URL]              $([ "$SCRIPT_LANG" = "de" ] && echo "Download von angegebener URL" || echo "Download from specified URL")
    $0                    $([ "$SCRIPT_LANG" = "de" ] && echo "Download von URL aus Zwischenablage" || echo "Download from clipboard URL")
    $0 -h, --help         $([ "$SCRIPT_LANG" = "de" ] && echo "Diese Hilfe anzeigen" || echo "Show this help")

$(msg "help_examples")
    $0 https://youtube.com/watch?v=...
    $0 https://instagram.com/p/...
    $0 https://tiktok.com/@user/video/...
    $0  # $([ "$SCRIPT_LANG" = "de" ] && echo "Nutzt Link aus Clipboard" || echo "Uses link from clipboard")
...
$(msg "help_features")
    - $([ "$SCRIPT_LANG" = "de" ] && echo "Video-Download in wählbarer Qualität (Best/1080p/720p/480p)" || echo "Video download in selectable quality (Best/1080p/720p/480p)")
    - $([ "$SCRIPT_LANG" = "de" ] && echo "Audio-Only Download (MP3)" || echo "Audio-only download (MP3)")
    - $([ "$SCRIPT_LANG" = "de" ] && echo "Automatische Duplikat-Erkennung" || echo "Automatic duplicate detection")
    - $([ "$SCRIPT_LANG" = "de" ] && echo "Optional: Bearbeitung mit Shotcut" || echo "Optional: Edit with Shotcut")
    - $([ "$SCRIPT_LANG" = "de" ] && echo "Desktop-Benachrichtigungen" || echo "Desktop notifications")

$(msg "help_dependencies")
    - yt-dlp ($(msg "help_required"))
    - xclip $([ "$SCRIPT_LANG" = "de" ] && echo "oder" || echo "or") wl-paste ($(msg "help_optional"), $(msg "help_clipboard"))
    - shotcut ($(msg "help_optional"), $(msg "help_editing"))
    - notify-send ($(msg "help_optional"), $(msg "help_notifications"))

$([ "$SCRIPT_LANG" = "de" ] && echo "DOWNLOAD-ORDNER:" || echo "DOWNLOAD FOLDERS:")
    Videos: $DOWNLOAD_DIR
    Audio:  $AUDIO_DIR
$([ "$SCRIPT_LANG" = "de" ] && echo "LOG-DATEI:" || echo "LOG FILE:") $LOG_FILE
EOF
}
...
clean_url() {
    ...
    local tracking_params=(
        ...
        "igsh" "igshid" "ig_rid" "ig_web_copy_link" "ig_web_button_share"
        "fbclid" "gclid" "mc_cid" "mc_eid" "_ga" "_gl"
        "ref_src" "ref_url" "s" "t"
        "si" "feature" "app" "ab_channel"
        "_r" "u_code" "social_share"
        "context" "share_id"
    )
    ...
    if [[ "$url" == *"?"* ]]; then
        ...
        for param in "${PARAMS[@]}"; do
            ...
            for track in "${tracking_params[@]}"; do
                if [[ "$key" == "$track" ]] || [[ "$key" == "$track"* ]]; then
                    is_tracking=1
                    break
                fi
            done
...
rotate_log() {
    if [ -f "$LOG_FILE" ]; then
        ...
        if [ "$line_count" -gt "$LOG_MAX_LINES" ]; then
            echo "📋 $(msg "msg_log_rotate") (${line_count} $([ "$SCRIPT_LANG" = "de" ] && echo "Zeilen" || echo "lines"))..."
            mv "${LOG_FILE}.2.old" "${LOG_FILE}.3.old" 2>/dev/null || true
            mv "${LOG_FILE}.1.old" "${LOG_FILE}.2.old" 2>/dev/null || true
...
# Link aus Argument oder Clipboard
LINK="${1:-}"
